...
<div class="col-sm-12 col-md-6 col-md-offset-3">
 ...
 <div class="font-group">
			...
			<input class="form-control input-lg" required autocomplete="off" type="search"  id="sch_value" placeholder="Search with School Name" name="school_name"/>
             <div id="school_name" style="color:#F00; text-align:center; line-height:30px;"></div>
		</div>
...
<div class="font-group">
				 ...
				 <select name="cat_id" id="yourid" class="form-control input-lg input2">
                  <option value="">Select Category</option>
                 <?php foreach ($categories as $cat) { ?>
                 <option value="<?php echo $cat->cat_id; ?>"><?php  echo $cat->category; ?></option>
                 <?php }?>
                 </select>
				 <div id="category_name" style="color:#F00;text-align:center; line-height:30px;"></div>
			<input type="hidden" id="cat_value" value="" name="category">
...
<input class="btn btn-block btn-primary btn-lg" onClick="form_submit()" style="margin-top:20px; border-radius:0px;" type="button" value="Search"/>
...
function get_data(url_oo){
	
document.getElementById('main_data_p1').innerHTML='<img src="1445974136-loading.gif" />'
	...
	if (getSecretAccessKey() == "AWS Secret Access Key") {
	  ...
	  return;
        ...
        var unsignedUrl = 'http://webservices.amazon.com/onca/xml?Service=AWSECommerceService&Version=2009-03-31&Operation=ItemLookup&ItemId='+url_oo+'&ResponseGroup=Images,ItemAttributes,Offers,Reviews&AssociateTag=textbookpit-20';
        if (unsignedUrl == "") {
	...
	if (matches == null) {
      ...
      //  stringToSignArea.value = stringToSign;
	   ...
	   document.getElementById('main_url1').innerHTML=signedUrl;
	   $.ajax({
		url: 'test.php',
		type: 'get',
		dataType: 'html',
		...
		success: function(json) {
			document.getElementById('main_data_p1').innerHTML=json;
...
function showResult(str) {
  ...
  xmlhttp.onreadystatechange=function() {
    if (this.readyState==4 && this.status==200) {
	  ...
	  document.getElementById("livesearch").innerHTML= document.getElementById("livesearch").innerHTML+'<img src="images/Loading.gif" id="loda_imag" />';
  ...
  xmlhttp.send();
	...
	if (getSecretAccessKey() == "AWS Secret Access Key") {
	  ...
	  return;
        ...
        var unsignedUrl = 'http://webservices.amazon.com/onca/xml?Service=AWSECommerceService&Version=2009-03-31&Operation=ItemSearch&SearchIndex=Books&AssociateTag=textbookpit-20';
        if (unsignedUrl == "") {
	  alert("Please provide a URL");
	  return;
	}
unsignedUrl=unsignedUrl+'&Keywords='+str;
	var lines = unsignedUrl.split("\n");
	unsignedUrl = "";
	for (var i in lines) { unsignedUrl += lines[i]; }
	...
	if (matches == null) {
      ...
      //  stringToSignArea.value = stringToSign;
	   ...
	   $.ajax({
		url: 'index.php/classified_ads/search_module_amozon/',
		type: 'get',
		dataType: 'html',
		...
		success: function(json) {
			...
			document.getElementById("livesearch").innerHTML=document.getElementById("livesearch").innerHTML+json;
			document.getElementById('loda_imag').style.display='none';
		}
	});
	        ...
	        <script src="js/index.js"></script><form id="filter_form">               
