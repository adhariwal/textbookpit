...
private $limit = 25;
...

    {
        ...
        $statement = "`classified_ads` JOIN `classified_category` ON `classified_ads`.`cat_id`=`classified_category`.`cat_id` JOIN `classified_sub_category` ON `classified_ads`.`sub_cat_id`=`classified_sub_category`.`sub_cat_id` WHERE `ad_status` = 1 AND `cat_status` = 1 AND `sub_cat_status` = 1 ORDER BY `ads_id` DESC";
	...
	$data='<div class="list-subgroups">';
	 ...
	 foreach ($query->result() as $row) {
		   $on="put_data_in('".$row->COLLEGE."','".$row->ID."')";
                $data .= '<a class="list-subgroup-item"  onclick="'.$on.'">'.$row->COLLEGE.'</a>';
            }
			 $data .='</div>';
			echo $data;
...
public function search_module2(){
    ...
    public function ads(){
        ...
        $statement = "`classified_ads` JOIN `classified_category` ON `classified_ads`.`cat_id`=`classified_category`.`cat_id` JOIN `classified_sub_category` ON `classified_ads`.`sub_cat_id`=`classified_sub_category`.`sub_cat_id` WHERE `ad_status` = 1 AND `cat_status` = 1 AND `sub_cat_status` = 1 ";
 ...
 if(isset($_GET['isbn'])){
        ...
        if(isset($_GET['subcatid'])){
		   ...
		   if(isset($_GET['school_name'])){
            ...
            $where .= "AND classified_ads.school_name = '".$school_name."' ";
